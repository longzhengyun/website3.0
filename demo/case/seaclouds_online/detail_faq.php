...
<div class="sc-wrap">
	...
	<div class="sc-layout sc-detail clearfix">
		<ul class="sc-layout-aside">
			...
			<li class="cur"><a href="detail_faq.php" class="tags-dashed">FAQ</a></li>
		...
		<div class="sc-layout-main">
			<dl class="detail-item">
				...
				<dd>
					<div class="faq-item">
						<h3 class="faq-title">1.大赛数据抽样方式？</h3>
						<div class="faq-cont">
							<p>首先是抽取用户，从移动端对垂直行业商品在数据周期内（2014年11月18日 至 2014年12月19日） 有行为用户中抽取指定数量的用户，然后抽取这些用户在所有商品上的行为作为用户行为训练数据；抽取这些用户有行为的所有垂直行业商品作为商品子集，用户抽样的总体思想是随机抽样，同时保证预测日有购买的商品在训练周期内也是活跃商品（即排除预测日偶发购买的用户）。</p>
							<p>训练数据集对应的时间区间为11.18~12.18，需要选手预测12.19用户商品购买对。</p>
						</div>
					</div>
					<div class="faq-item">
						<h3 class="faq-title">2.大赛数据抽样方式？</h3>
						<div class="faq-cont">
							<p>首先是抽取用户，从移动端对垂直行业商品在数据周期内（2014年11月18日 至 2014年12月19日） 有行为用户中抽取指定数量的用户，然后抽取这些用户在所有商品上的行为作为用户行为训练数据；抽取这些用户有行为的所有垂直行业商品作为商品子集，用户抽样的总体思想是随机抽样，同时保证预测日有购买的商品在训练周期内也是活跃商品（即排除预测日偶发购买的用户）。</p>
							<p>训练数据集对应的时间区间为11.18~12.18，需要选手预测12.19用户商品购买对。</p>
						</div>
					...
					<div class="faq-item">
						<h3 class="faq-title">3.大赛数据抽样方式？</h3>
						<div class="faq-cont">
							<p>首先是抽取用户，从移动端对垂直行业商品在数据周期内（2014年11月18日 至 2014年12月19日） 有行为用户中抽取指定数量的用户，然后抽取这些用户在所有商品上的行为作为用户行为训练数据；抽取这些用户有行为的所有垂直行业商品作为商品子集，用户抽样的总体思想是随机抽样，同时保证预测日有购买的商品在训练周期内也是活跃商品（即排除预测日偶发购买的用户）。</p>
							<p>训练数据集对应的时间区间为11.18~12.18，需要选手预测12.19用户商品购买对。</p>
						</div>
